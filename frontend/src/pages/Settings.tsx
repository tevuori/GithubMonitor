  ...
  return (
    <div className="space-y-6" data-testid="settings-page">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-300">Repository Settings</h1>
      </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-700 dark:text-gray-300">{settings.private ? 'Private' : 'Public'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${settings.archived ? 'bg-red-500' : 'bg-green-500'}`}></div>
                      <span className="text-sm text-gray-700 dark:text-gray-300">{settings.archived ? 'Archived' : 'Active'}</span>
                    </div>
                  </div>

                  <div className="border-t pt-6">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-gray-300 mb-4">Features</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {[
                  ...
                  <div className="border-t pt-6">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-gray-300 mb-4">Merge Settings</h3>
                  ...
                  <div className="border-t pt-6">
                    <h3 className="text-lg font-medium text-gray-900 dark:text-gray-300 mb-4">Your Permissions</h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-3">
                    {protection.required_status_checks && (
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">Required Status Checks</h4>
                    ...
                    {protection.enforce_admins && (
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">Enforce on Admins</h4>
                        <p className="text-sm text-gray-500">
                    ...
                    {protection.required_pull_request_reviews && (
                      <div>
                        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">Required Reviews</h4>
                        <p className="text-sm text-gray-500">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-sm font-medium text-gray-900 dark:text-gray-300 hover:text-blue-600"
                          >
                            {collab.login}
                          </a>
          ...
          {activeTab === 'webhooks' && (
            <div>
              {webhooksLoading ? (
                ...
                <div className="space-y-4">
                  {webhooks.map((hook) => (
                    <div key={hook.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${hook.active ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-800'}`}></span>
                          <span className="font-medium text-gray-900 dark:text-gray-300">{hook.name}</span>
                        </div>
                        <span className={`text-xs ${hook.active ? 'text-green-600' : 'text-gray-500'}`}>
                          {hook.active ? 'Active' : 'Inactive'}
                        </span>
                      </div>
                      <p className="text-sm text-gray-600 dark:text-gray-300 font-mono truncate">{hook.config.url}</p>
                      <div className="flex flex-wrap gap-1 mt-2">
                        {hook.events.slice(0, 5).map((event) => (
                          <span key={event} className="px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded text-xs">
                            {event}
                          </span>
                        ))}
