      ...
      <div className="mt-6 bg-white shadow rounded-lg divide-y divide-gray-200">
        ...
        {runs?.map((run: any) => (
          <div key={run.id} className="p-4 flex items-center justify-between">
            <div>
              <a href={run.html_url} target="_blank" rel="noreferrer" className="text-sm font-medium text-blue-600 hover:underline">{run.name}</a>
              <p className="text-xs text-gray-500 mt-1">{run.head_branch} · {run.event} · {new Date(run.updated_at).toLocaleString()}</p>
            </div>
