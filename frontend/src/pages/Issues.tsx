...
const Issues: React.FC = () => {
  ...
  const renderBody = (body: string | null) => {
    if (!body) return <p className="text-gray-500 dark:text-gray-400 italic">No description provided.</p>;
    const rendered = body
      .replace(/```(\w+)?\n([\s\S]*?)```/g, '<pre class="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg my-2 overflow-x-auto text-sm font-mono">$2</pre>')
      .replace(/`([^`]+)`/g, '<code class="bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded text-sm">$1</code>')
      .replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>')
      .replace(/\*([^*]+)\*/g, '<em>$1</em>')
      .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" target="_blank" rel="noreferrer" class="text-blue-600 dark:text-blue-400 hover:underline">$1</a>')
      ...
      .replace(/\n\n/g, '</p><p class="mt-3">')
      .replace(/\n/g, '<br/>');
    return <div className="prose prose-sm dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: `<p>${rendered}</p>` }} />;
  };
    ...
    <div className="space-y-6" data-testid="issues-page">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Track and manage issues for your repositories</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
          <select
            className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
            value={selectedRepo}
            onChange={e => { setSelectedRepo(e.target.value); setSelectedIssue(null); setShowCreate(false); }}
      ...
      {showCreate && owner && repo && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-4" data-testid="create-issue-panel">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Create issue</h2>
            <span className="text-xs text-gray-500 dark:text-gray-400 font-mono">{owner}/{repo}</span>
          </div>
          <div className="grid grid-cols-1 gap-3">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-[120px]"
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => { setShowCreate(false); setNewTitle(''); setNewBody(''); }}
      ...
      {issues && (
        <div className="flex items-center gap-6 text-sm">
          <span className="text-gray-600 dark:text-gray-400">
            <span className="font-semibold text-gray-900 dark:text-white">{filteredIssues?.length}</span> issues
          </span>
          <span className="text-green-600 dark:text-green-400">{issues.filter(i => i.state === 'open').length} open</span>
          <span className="text-gray-500 dark:text-gray-400">{issues.filter(i => i.state === 'closed').length} closed</span>
        </div>
      )}

      <div className="flex gap-6">
        {/* Issues List */}
        <div className={`bg-white dark:bg-gray-800 shadow rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 ${selectedIssue ? 'w-2/5' : 'w-full'}`}>
          {issuesLoading && (
          ...
          {!selectedRepo && !issuesLoading && (
            <div className="p-8 text-center">
              <p className="text-gray-400 dark:text-gray-500">Select a repository to view issues</p>
            </div>
          )}
            ...
            {filteredIssues?.map((issue) => (
              <div
                ...
                className={`px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-colors ${
                  selectedIssue?.number === issue.number ? 'bg-blue-50 dark:bg-blue-900/20 border-l-2 border-l-blue-500' : ''
                }`}
                onClick={() => setSelectedIssue({ owner, repo, number: issue.number })}
                data-testid={`issue-${issue.number}`}
              >
                <div className="flex items-start gap-3">
                  <div className={`mt-0.5 ${issue.state === 'open' ? 'text-green-600 dark:text-green-400' : 'text-purple-600 dark:text-purple-400'}`}>
                    {issue.state === 'open' ? (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                      ...
                      {issue.labels?.slice(0, 3).map((label) => (
                        <span key={label.id} className="px-1.5 py-0.5 rounded-full text-xs"
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">opened {formatRelativeTime(issue.created_at)} by {issue.user?.login}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* Issue Details Panel */}
        {selectedIssue && (
          <div className="w-3/5 bg-white dark:bg-gray-800 shadow rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
            <div className="px-4 py-3 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <h2 className="font-semibold text-gray-900 dark:text-white">Issue Details</h2>
              <button onClick={() => setSelectedIssue(null)} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="overflow-y-auto max-h-[calc(100vh-300px)]">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                  <div className="flex items-start gap-2">
                    ...
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{issueDetails.title}</h2>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4 mt-4 text-sm">
                    {issueDetails.labels && issueDetails.labels.length > 0 && (
                      <div>
                        ...
                        {issueDetails.labels.map((label: any) => (
                          <span key={label.id} className="px-2 py-0.5 rounded-full text-xs ml-1"
                            style={{ backgroundColor: `#${label.color}22`, color: `#${label.color}` }}>
                            {label.name}
                          </span>
                        ))}
                    ...
                    {issueDetails.assignees && issueDetails.assignees.length > 0 && (
                      <div className="flex items-center gap-1">
                        ...
                        {issueDetails.assignees.map((a: any) => (
                          <img key={a.login} src={a.avatar_url} className="w-5 h-5 rounded-full" title={a.login} alt="" />
                        ))}
                      </div>
                    )}
                    {issueDetails.milestone && (
                      <div>
                        ...
                        <span className="text-gray-700 dark:text-gray-300">{issueDetails.milestone.title}</span>
                      </div>
                    )}
                  </div>
                  <a href={issueDetails.html_url} target="_blank" rel="noreferrer"
                    className="inline-block mt-4 text-sm text-blue-600 dark:text-blue-400 hover:underline">View on GitHub →</a>
                </div>
                <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                      <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                        <span className="font-medium text-gray-700 dark:text-gray-300">{issueDetails.user?.login}</span> commented {formatRelativeTime(issueDetails.created_at)}
                      </div>
                ...
                {issueDetails.comments > 0 && (
                  <div className="p-4">
                    <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">Comments ({issueDetails.comments})</h3>
                    {commentsLoading ? (
                      <div className="text-center py-4"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-blue-500 mx-auto"></div></div>
                      ...
                      <div className="space-y-4">
                        {comments.map((comment) => (
                          <div key={comment.id} className="flex items-start gap-3">
                            ...
                            <div className="flex-1 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                              <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                                <span className="font-medium text-gray-700 dark:text-gray-300">{comment.user?.login}</span> commented {formatRelativeTime(comment.created_at)}
                              </div>
                      ...
                      <p className="text-gray-500 dark:text-gray-400 text-sm">Failed to load comments</p>
                    )}
                  </div>
                )}
              </div>
