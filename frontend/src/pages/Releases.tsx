...
interface Release {
  ...
  tarball_url: string;
  zipball_url: string;
  assets: Array<{ id: number; name: string; size: number; download_count: number; browser_download_url: string; content_type: string }>;
}
...
const Releases = () => {
  ...
  const [formData, setFormData] = useState({ tag_name: '', name: '', body: '', draft: false, prerelease: false, target_commitish: 'main' });
  ...
  const createReleaseMutation = useMutation({
    ...
    onError: (error: any) => { toast.error(error.response?.data?.error || 'Failed to create release'); },
  });
    ...
    <div className="space-y-6" data-testid="releases-page">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-4">
        ...
        <select
          ...
          className="w-full md:w-64 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500"
      ...
      {releasesLoading ? (
        ...
        <div className="space-y-4">
          {releases.map((release) => (
            <div key={release.id} className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden border border-gray-200 dark:border-gray-700">
              <div className="p-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{release.name || release.tag_name}</h2>
                      <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded text-sm font-mono">{release.tag_name}</span>
                      {release.draft && <span className="px-2 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded text-sm">Draft</span>}
                      {release.prerelease && <span className="px-2 py-0.5 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded text-sm">Pre-release</span>}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a href={release.html_url} target="_blank" rel="noreferrer"
                      className="px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-500 text-sm">View on GitHub</a>
                  </div>
                </div>
                {release.body && (
                  <div className="mt-4">
                    <pre className="whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                      {release.body}
                    </pre>
                  </div>
                )}
                {release.assets.length > 0 && (
                  <div className="mt-4">
                    <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Assets ({release.assets.length})</h3>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-8 text-center border border-gray-200 dark:border-gray-700">
          ...
          <h3 className="text-lg font-medium text-gray-700 dark:text-gray-300 mb-2">No releases yet</h3>
          <p className="text-gray-500 dark:text-gray-400 mb-4">Create your first release to share your project.</p>
          <button onClick={() => setShowCreateModal(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-500">Create Release</button>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-gray-700">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Create New Release</h2>
            ...
            <form onSubmit={(e) => { e.preventDefault(); createReleaseMutation.mutate(formData); }} className="p-4 space-y-4">
              {[
                { label: 'Tag name *', key: 'tag_name', placeholder: 'v1.0.0', required: true, testId: 'release-tag-input' },
                { label: 'Release title', key: 'name', placeholder: 'Release title' },
                { label: 'Target branch', key: 'target_commitish', placeholder: 'main' },
              ].map(({ label, key, placeholder, required, testId }) => (
                <div key={key}>
              ...
              <div>
                ...
                <textarea rows={6} value={formData.body}
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={formData.prerelease}
                ...
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={formData.draft}
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit" disabled={createReleaseMutation.isPending}
